...
from . import utils, layers, layerspp, dense_layer, vmamba
...
VSSandCNN_Block = vmamba.VSSandCNN_Block
...
class HF_guided_CA(nn.Module):
    def __init__(self, in_channel, norm_groups=32):
        ...
        self.q = conv1x1(9, in_channel, bias=False)
        self.kv = conv1x1(in_channel, in_channel * 2, bias=False)
        self.out = conv1x1(in_channel, in_channel)
...
@utils.register_model(name='vm_wavelet_ncsnpp')
class VM_WaveletNCSNpp(NCSNpp):
    """NCSN++ model"""
    ...
    def __init__(self, config):
        ...
        self.use_HF_CrossAttn = use_HF_CrossAttn = config.use_HF_CrossAttn
        ...
        VSSBlock = functools.partial(VSSandCNN_Block, temb_dim=nf * 4, zemb_dim=z_emb_dim)
        if use_HF_CrossAttn:
            HF_CrossAttn = functools.partial(HF_guided_CA)
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                modules.append(VSSBlock(hidden_dim=in_ch))
        ...
        for i_level in reversed(range(num_resolutions)):
            for i_block in range(num_res_blocks + 1):
                ...
                # HF_CrossAttn
                if use_HF_CrossAttn and i_block > 0 and i_level != 0 and i_level != num_resolutions - 1:
                   modules.append(HF_CrossAttn(in_channel=hs_c[-1], norm_groups=min(hs_c[-1] // 4, 32)))
    ...
    def forward(self, x, time_cond, z):
        ...
        if self.use_HF_CrossAttn:
           _, cond_wavelet = torch.chunk(x, 2, dim=1)
           cond_ll, cond_h = cond_wavelet[:, :3], cond_wavelet[:, 3:]
           cond_hs = [cond_h]
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                hs.append(h)
                if self.use_HF_CrossAttn and i_level != 0 and i_level != self.num_resolutions - 1:
            ...
            if i_level != self.num_resolutions - 1:
                ...
                hs.append(h)
                if self.use_HF_CrossAttn:
                    xll, xlh, xhl, xhh = self.dwt(cond_ll)
                    cond_ll = xll / 2.0
                    cond_h = torch.cat([xlh, xhl, xhh], dim=1) / 2.0
                    # cond_hs.append(cond_h)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                ...
                # HF_CrossAttn
                if self.use_HF_CrossAttn and i_block > 0 and i_level != 0 and i_level != self.num_resolutions - 1:
                   skip_h = modules[m_idx](skip_h, cond_hs.pop())
                   m_idx += 1

                h = modules[m_idx](torch.cat([h, skip_h], dim=1), temb, zemb)
                m_idx += 1
